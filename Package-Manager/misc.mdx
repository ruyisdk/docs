...
默认情况下，遥测数据收集级别是 `local`，意味着每一次 `ruyi` 调用都会记录一些非敏感的操作数据和 `ruyi` 的一些状态信息，但不会自动上传到远程服务器。

在终端界面首次运行 `ruyi` 时,`ruyi` 会询问您的数据收集偏好,并将您的选择写入用户配置文件。

如果您希望将遥测数据发送到远程服务器(级别为 `on`),数据将每周发送至由`RuyiSDK`管理的在中国大陆的远程服务器。上传会发生在随机的一天,由匿名安装 ID 决定。
